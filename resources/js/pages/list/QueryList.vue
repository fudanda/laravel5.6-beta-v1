    ...
    getData() {
      this.$api.getfuli().then(res => {
        ...
        for (let i = 0; i < res.results.length; i++) {
          console.log(res.results[i].type);
          this.dataSource.push({
            key: res.results[i]._id,
            no: res.results[i]._id,
            description: res.results[i].type,
            callNo: res.results[i].lijinshanmx,
            status: res.results[i].lijinshanmx,
            updatedAt: res.results[i].lijinshanmx
          });
        }
      });
    }
